...
class PacketProcessor:
    ...
    def find_loss(self, print_statistics: bool = True) -> None:
        ...
        self._sent_groups_ids = split_packets_by_groups(sent_packets, PacketType.sent)

        received_packets: list[Packet] = rdpcap(config.RECEIVED_FILE).res
        self._received_count = len(received_packets)
        self._received_groups_ids = split_packets_by_groups(received_packets, PacketType.received)
    ...
    def print_statistics(
        self, sent_not_received: int, received_not_sent: int
    ) -> None:
        ...
        line += f"Time spent on packet compare: {(self._end_datetime - self._start_compare_datetime)}\n"
        line += "=" * 15 + "\n"
        print(line)
